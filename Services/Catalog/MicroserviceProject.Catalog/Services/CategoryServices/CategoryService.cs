...
namespace MicroserviceProject.Catalog.Services.CategoryServices
{
    public class CategoryService : ICategoryService
    {
        ...
        public CategoryService(IMapper mapper, IDatabaseSettings _databaseSettings)
        {
            ...
            _categoryCollection = database.GetCollection<Category>(_databaseSettings.CategoryCollectionName);     // tabloyu getirme
            _mapper = mapper;
        }

        public async Task CreateCategoryAsync(CreateCategoryDto createCategoryDto)
        {
            var value = _mapper.Map<Category>(createCategoryDto); //category entitysi ile createCategoryDto'dan gelen parametreleri eşleştir
